...
add_config()
{
    ...
    #Update DHCP config
    uci add_list dhcp.@dnsmasq[0].addnhosts=/tmp/block.hosts > /dev/null 2>&1 && uci commit
...
update_blocklist()
{
    ...
    then
        ...
        egrep -v "^[[:space:]]*$" /etc/white.list | awk '/^[^#]/ {sub(/\r$/,"");print $1}' | grep -vf - /tmp/block.build.before > /tmp/block.hosts
    else
    ...
    then
        ...
        sed -i -re "s/^(${safe_pattern}) (.*)$/\1 \2\n${safe_addition} \2/g" /tmp/block.hosts
    fi
}
...
remove_config()
{
    ...
    # Remove addnhosts
    uci del_list dhcp.@dnsmasq[0].addnhosts=/tmp/block.hosts > /dev/null 2>&1 && uci commit
